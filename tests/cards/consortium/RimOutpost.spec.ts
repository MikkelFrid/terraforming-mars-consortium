...
import {RimOutpost} from '../../../src/server/cards/consortium/RimOutpost';
...
describe('RimOutpost', () => {
  it('requires an open frontier sector', () => {
    ...
    expect(Frontier.countOpenSectors(game)).eq(1);
    expect(card.canPlay(player)).is.true;
  });

  it('places a frontier tile and raises M€ production 2 steps', () => {
    const card = new RimOutpost();
    const [game, player] = testGame(2, {consortiumExpansion: true, boardName: BoardName.CONSORTIUM});
    unlockBridgeSector(game.board.spaces, 0);
    game.megastructuresData!.structures.find((s) => s.id === 'bridge-0')!.completed = true;
    ...
    expect(select.spaces[0].tile?.tileType).eq(TileType.RIM_OUTPOST);
  });
