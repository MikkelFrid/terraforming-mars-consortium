...
function writeReport(
  ...
  lines.push('');
  lines.push('Branch: `feat/consortium-validation`');
  lines.push(`Date: ${now}`);
  ...
  lines.push('- **Board note:** Consortium map currently has **0 ocean spaces**. `PlaceOceanTile`');
  lines.push('  soft-skips when no spaces exist (otherwise Prelude ocean cards crash).');
  lines.push('');
  ...
  lines.push('multi-player interaction, rare tags, or ocean placement on the Consortium');
  lines.push('board (which currently has **zero** `SpaceType.OCEAN` spaces) will appear here.');
  lines.push('');
