...
MEGASTRUCTURES = [
    ("bridge-0.png", "Bridge (Sector 0)", "Always in play. Completing it unlocks frontier sector 0 and converts that sector’s chasms to land."),
    ("bridge-1.png", "Bridge (Sector 1)", "Always in play. Completing it unlocks frontier sector 1 and converts that sector’s chasms to land."),
    ("bridge-2.png", "Bridge (Sector 2)", "Always in play. Completing it unlocks frontier sector 2 and converts that sector’s chasms to land."),
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* Swatches match board hex overlays in assets/hex_*.png */
    .t-chasm {{ background: #121212; }}
    .t-crater {{ background: #1a3a5c; }}
    .t-highland {{ background: #6b3f1e; }}
...
<body>
  <div class="wrap">
    ...
    <div class="map-gallery">
      <figure class="map-card" id="massif">
      ...
      <figure class="map-card" id="rift-basin">
      ...
      <figure class="map-card" id="archipelago">
    ...
    <table>
      <thead>
        <tr><th>Terrain</th><th>Look on the board</th><th>Rules</th></tr>
      </thead>
      <tbody>
        <tr>
          <td><span class="terrain t-chasm"></span> Chasm</td>
          <td class="terrain-look">
            <img class="terrain-hex" src="/assets/hex_chasm.png" alt="Chasm hex" width="46" height="50" />
            Black empty hex
          </td>
          <td>Nothing may be placed. When that sector’s <strong>Bridge</strong> completes, those chasms convert to normal land (and the locked frontier unlocks).</td>
        </tr>
        <tr>
          <td><span class="terrain t-crater"></span> Crater field</td>
          <td class="terrain-look">
            <img class="terrain-hex" src="/assets/hex_crater_field.png" alt="Crater field hex" width="46" height="50" />
            Blue hex with white crater circles
          </td>
          <td>When you claim the space (city, greenery, special tile, etc.), gain <strong>2 iridium</strong> from the bank <em>once</em> per space.</td>
        </tr>
        <tr>
          <td><span class="terrain t-highland"></span> Highland</td>
          <td class="terrain-look">
            <img class="terrain-hex" src="/assets/hex_highland.png" alt="Highland hex" width="46" height="50" />
            Brown hex with white elevation arcs
          </td>
        ...
        <tr>
          ...
          <td class="terrain-look">Padlock on locked hexes</td>
          <td>Spaces start locked. Completing the matching <strong>Bridge</strong> unlocks that sector for normal play.</td>
        </tr>
      </tbody>
    </table>
    <p>Oceans, cities, greenery, and adjacency work as in the base game unless a card says otherwise. The base-game card <em>Domed Crater</em> places a normal city — it is <strong>not</strong> a Consortium crater field.</p>

    <h2 id="iridium">3. Iridium</h2>
    <div class="icon-row">
      <div class="icon-card">
        <img src="/assets/resources/iridium.png" alt="Iridium resource icon" width="64" height="64" />
        <strong>Iridium</strong><br />Player resource · 5 M€
      </div>
    </div>
    <ul>
      <li><strong>Bank:</strong> 28 tokens. When empty, you cannot gain more until some returns.</li>
      <li><strong>Value:</strong> 1 iridium = <strong>5 M€</strong> when paying for eligible cards and megastructure segments.</li>
      ...
      <li><strong>No iridium production.</strong> Recurring gains come from effects such as <em>Siderophile Extraction</em> (1 iridium at generation start if the bank has any — deliberately <strong>no building tag</strong>, so Robotic Workforce does not copy it) and Mohole contributors.</li>
    </ul>
    ...
    <ul>
      <li>Claiming a crater-field space (<strong>2 iridium</strong>, once per space)</li>
      ...
      <li>Standard project <em>Core Sampling</em> (6 M€ → 1 iridium)</li>
    </ul>
...
{megas}
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>Unlocks that frontier sector; chasms in the sector become land</td>
          <td>+2 M€ production per segment owned</td>
        </tr>
        <tr>
          <td><strong>Space Elevator</strong></td>
          <td>6</td>
          <td>All: −2 M€ on space-tag cards</td>
          <td>+1 titanium production per 2 segments owned (min 1)</td>
        </tr>
        <tr>
          <td><strong>L1 Magnetic Shield</strong></td>
          <td>6</td>
          <td>All: greenery costs 1 fewer plant</td>
          <td>+1 plant production per 2 segments owned (min 1)</td>
        </tr>
        <tr>
          <td><strong>Mohole</strong></td>
          <td>6</td>
          <td>All: +1 heat production</td>
          <td>+2 iridium per segment now; +1 iridium each generation</td>
        </tr>
        <tr>
          <td><strong>Solar Mirror</strong></td>
          <td>6</td>
          <td>Temperature +1 step</td>
          <td>+1 heat production per 2 segments owned (min 1)</td>
        </tr>
        <tr>
          <td><strong>Arcology</strong></td>
          <td>6</td>
          <td>All: +1 M€ production</td>
          <td>+2 extra VP per segment at end (stacks with base segment VP)</td>
        </tr>
      </tbody>
    </table>
    ...
    <ul>
      ...
      <li>On completion: contributors score <strong>2 VP per segment</strong> they paid for; the keystone player scores an extra <strong>+3 VP</strong>.</li>
    ...
    <p>No. That card has no building tag on purpose. It also has no production box — its recurring iridium is a generation-start effect.</p>
    <h3>Is Domed Crater a crater field?</h3>
    <p>No. <em>Domed Crater</em> is a base-game city card. Consortium crater fields are the blue hexes with white crater circles on the map.</p>
    ...
    <p>Each sector stays locked until its Bridge is finished. Then the frontier unlocks and that sector’s chasms become land.</p>
...
def main() -> None:
    ...
    required = [
        ...
        ROOT / "assets/hex_chasm.png",
        ROOT / "assets/hex_crater_field.png",
        ROOT / "assets/hex_highland.png",
    ]
